          ...
          handler: (dataOpsi) => {
            ...
            this.databaseService.updateBarang(this.dataBarang).then(() => {
              if (dataOpsi == 0) {
                ...
                this.notificationService.scheduleNotification('1', 'Pengingat!', `Jangan lupa ${this.dataBarang.nama_barang.toLowerCase()} ${this.dataBarang.status.toLowerCase()}`, this.id, new Date(date.getTime()), `/barang/show/${this.id}`);
              } else if (dataOpsi == 1) {
                this.notificationService.cancelNotification(this.id);
              }
              this.dataSharingService.refresh();
